<template>
	<div class="gms-input-wrapper" :class="{ 'has-error': hasError, disabled: disabled }">
		...
		<div class="gms-input-container">
			<i v-if="prefixIcon" class="fas gms-input-icon prefix" :class="prefixIcon"></i>

			<input :id="inputId" :type="type" :value="modelValue" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :maxlength="maxlength" :min="min" :max="max" :step="step" class="gms-input" :class="{ 'has-prefix': prefixIcon, 'has-suffix': suffixIcon || showPasswordToggle }" @input="handleInput" @blur="handleBlur" @focus="handleFocus" />

			<i v-if="showPasswordToggle" :class="type === 'password' ? 'fa-eye' : 'fa-eye-slash'" class="fas gms-input-icon suffix password-toggle" @click="togglePassword"></i>

			<i v-else-if="suffixIcon" :class="suffixIcon" class="gms-input-icon suffix"></i>

			<button v-if="clearable && modelValue" type="button" class="gms-input-clear" @click="handleClear">
