	...
	<div class="gms-table-wrapper">
		...
		<div class="gms-table-container" :class="{ scrollable: scrollable }">
			<table class="gms-table">
				<thead>
					<tr>
						<th v-for="(column, index) in columns" :key="index" :class="column.class" :style="column.style">
							<div class="gms-table-th-content">
								{{ column.label }}
								<div class="gms-table-th-actions">
									<i v-if="column.filterable" class="fas fa-filter gms-table-filter-icon" @click.stop="handleFilterClick(column)" title="Lọc"></i>
									<i v-if="column.sortable" :class="getSortIcon(column.key)" class="fas gms-table-sort-icon" @click.stop="handleSort(column.key)" title="Sắp xếp"></i>
								</div>
							</div>
						</th>
					</tr>
				...
				<tbody>
					...
					<tr v-else v-for="(row, rowIndex) in paginatedData" :key="rowIndex" :class="getRowClass(row, rowIndex)" @click="handleRowClick(row, rowIndex)">
						<td v-for="(column, colIndex) in columns" :key="colIndex" :class="column.class">
							<slot :name="`cell-${column.key}`" :row="row" :column="column" :value="getCellValue(row, column)">
								<component v-if="column.component" :is="column.component" :row="row" :column="column" :value="getCellValue(row, column)" />
		...
		<div v-if="pagination && totalPages > 1" class="gms-table-pagination">
			<div class="gms-table-pagination-info">Hiển thị {{ startIndex + 1 }}-{{ endIndex }} trong tổng {{ totalItems }} mục</div>
			<div class="gms-table-pagination-controls">
				<button class="gms-table-pagination-btn" :disabled="currentPage === 1" @click="goToPage(currentPage - 1)">
					<i class="fas fa-chevron-left"></i>
				</button>

				<div class="gms-table-pagination-pages">
					<button v-for="page in visiblePages" :key="page" class="gms-table-pagination-page" :class="{ active: page === currentPage }" @click="goToPage(page)">
						{{ page }}
					</button>
				</div>

				<button class="gms-table-pagination-btn" :disabled="currentPage === totalPages" @click="goToPage(currentPage + 1)">
