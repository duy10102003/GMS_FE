				...
				<section class="card" id="step-customer">
					...
					<div class="form-grid three-cols">
						<div class="form-control">
							...
							<input v-model="form.customerName" type="text" placeholder="Nhập họ tên" :readonly="!!currentUser" />
						</div>
						<div class="form-control">
							<label>
								Số điện thoại
							...
							<input v-model="form.phone" type="tel" placeholder="Nhập số điện thoại" :disabled="!!currentUser" />
						</div>
						<div class="form-control">
							<label>Email</label>
							<input v-model="form.email" type="email" placeholder="Nhập email" :readonly="!!currentUser" />
						</div>
					</div>
				</section>
...
<script setup>
	...
	const isReadonlyUserInfo = computed(() => !!currentUser.value)
	...
	onMounted(async () => {
		const prefillFromUser = (user, customer = null) => {
			...
			form.customerName = user?.fullName || user?.name || customer?.fullName || customer?.name || form.customerName
			form.email = user?.email || customer?.email || form.email
			form.phone = user?.customerPhone || user?.phone || user?.phoneNumber || customerPhone || form.phone
		}

		if (currentUser.value?.userId) {
			const customerRes = await customerService.getByUserId(currentUser.value.userId || currentUser.value.id)
...
<style scoped>
	...
	.card::before {
		...
		position: absolute;
		inset: 0;
		background: radial-gradient(120% 120% at 10% 10%, rgba(255, 122, 0, 0.06), transparent), radial-gradient(120% 120% at 90% 0%, rgba(59, 130, 246, 0.05), transparent);
		pointer-events: none;
	}
