<template>
	<div class="create-service-ticket-view">
					...
					<form @submit.prevent="handleSubmit">
						...
						<div class="form-section">
							...
							<div class="search-container">
								...
								<div class="search-input-wrapper">
									<GmsInput v-model="customerSearch" placeholder="Tìm theo tên, số điện thoại, email..." prefix-icon="fa-search" :clearable="true" @input="searchCustomers" @focus="showCustomerDropdown = true" />
									<div v-if="showCustomerDropdown && customerSearchResults.length > 0" class="dropdown-results">
										<div v-for="customer in customerSearchResults" :key="customer.customerId" class="dropdown-item" @click="selectCustomer(customer)">
							...
							<div v-if="selectedCustomer" class="selected-item">
								...
								<GmsButton variant="outline" size="small" icon="fa-times" @click="clearCustomer">Xóa</GmsButton>
							</div>

							<div v-if="!selectedCustomer" class="form-row mt-3">
								<div class="col-md-4">
									<GmsInput v-model="newCustomer.customerName" label="Họ và tên *" placeholder="Nhập họ tên khách hàng" />
								</div>
								<div class="col-md-4">
						...
						<div class="form-section">
							...
							<div class="search-container">
								...
								<div class="search-input-wrapper">
									<GmsInput v-model="vehicleSearch" placeholder="Tìm theo tên xe, biển số..." prefix-icon="fa-search" :clearable="true" :disabled="!hasCustomer" @input="searchVehicles" @focus="showVehicleDropdown = true" />
									<div v-if="showVehicleDropdown && vehicleSearchResults.length > 0" class="dropdown-results">
										<div v-for="vehicle in vehicleSearchResults" :key="vehicle.vehicleId" class="dropdown-item" @click="selectVehicle(vehicle)">
							...
							<div v-if="selectedVehicle" class="selected-item">
								...
								<GmsButton variant="outline" size="small" icon="fa-times" @click="clearVehicle">Xóa</GmsButton>
							</div>

							<div v-if="!selectedVehicle" class="form-row mt-3">
								<div class="col-md-3">
									<GmsInput v-model="newVehicle.vehicleName" label="Tên xe *" placeholder="Honda Civic" />
								</div>
								<div class="col-md-3">
								...
								<div class="col-md-2">
									<GmsInput v-model="newVehicle.model" label="Model" placeholder="Civic" />
								</div>
							</div>
						</div>
						...
						<div class="form-section">
							...
							<div class="search-container">
								...
								<div class="search-input-wrapper">
									<GmsInput v-model="partSearch" placeholder="Tìm theo tên, mã phụ tùng..." prefix-icon="fa-search" :clearable="true" @input="searchParts" @focus="showPartDropdown = true" />
									<div v-if="showPartDropdown && partSearchResults.length > 0" class="dropdown-results">
										<div v-for="part in partSearchResults" :key="part.partId" class="dropdown-item" @click="addPart(part)">
							...
							<div v-if="selectedParts.length > 0" class="selected-items-list">
								<div v-for="(part, index) in selectedParts" :key="`${part.partId}-${index}`" class="selected-item">
									<div class="selected-info">
									...
									<GmsButton variant="outline" size="small" icon="fa-times" @click="removePart(index)">Xóa</GmsButton>
								</div>
							</div>
						</div>
						...
						<div class="form-section">
							<h5 class="section-title">
								...
								Dịch vụ garage (tùy chọn)
							</h5>

							<div class="search-container">
								<label class="form-label">Tìm kiếm dịch vụ</label>
								<div class="search-input-wrapper">
									<GmsInput v-model="serviceSearch" placeholder="Tìm theo tên dịch vụ..." prefix-icon="fa-search" :clearable="true" @input="searchServices" @focus="showServiceDropdown = true" />
									<div v-if="showServiceDropdown && serviceSearchResults.length > 0" class="dropdown-results">
										<div v-for="service in serviceSearchResults" :key="service.garageServiceId" class="dropdown-item" @click="addService(service)">
							...
							<div v-if="selectedServices.length > 0" class="selected-items-list">
								<div v-for="(service, index) in selectedServices" :key="`${service.garageServiceId}-${index}`" class="selected-item">
									<div class="selected-info">
									...
									<GmsButton variant="outline" size="small" icon="fa-times" @click="removeService(index)">Xóa</GmsButton>
								</div>
							</div>
						</div>
							...
							<div class="form-row">
								<div class="col-md-12">
									<div v-if="selectedTechnical" class="selected-item">
										...
										<GmsButton variant="outline" size="small" icon="fa-times" @click="clearTechnical">Xóa</GmsButton>
									</div>
									<GmsButton v-else variant="primary" icon="fa-user-plus" @click="showAssignModal = true">Chọn thợ</GmsButton>
								</div>
								<div v-if="selectedTechnical" class="col-md-12 mt-3">
									<GmsInput v-model="formData.assignDescription" label="Mô tả công việc" placeholder="Nhập mô tả công việc cho thợ..." />
								</div>
							</div>
						</div>
		...
		<GmsDialog v-model="showAssignModal" title="Chọn thợ kỹ thuật" size="large">
			<div class="assign-modal-content">
				<!-- Filter -->
				<div class="filter-section">
					<label class="form-label">Lọc theo trạng thái:</label>
					<div class="filter-buttons">
						<GmsButton variant="outline" :class="{ active: availabilityFilter === null }" @click="availabilityFilter = null">Tất cả</GmsButton>
						<GmsButton variant="outline" :class="{ active: availabilityFilter === true }" @click="availabilityFilter = true">Rảnh</GmsButton>
						<GmsButton variant="outline" :class="{ active: availabilityFilter === false }" @click="availabilityFilter = false">Đang bận</GmsButton>
					</div>
				</div>
				...
				<div class="dialog-actions">
					...
					<GmsButton variant="primary" :disabled="!selectedTechnical" @click="confirmTechnical">Xác nhận</GmsButton>
				</div>
			</div>
		</GmsDialog>
...
<script setup>
	...
	const searchParts = async () => {
		...
		try {
			...
			partSearchResults.value = (response.data || []).filter((part) => !selectedParts.value.find((sp) => sp.partId === part.partId))
		} catch (error) {
	...
	const searchServices = async () => {
		...
		try {
			...
			serviceSearchResults.value = (response.data || []).filter((service) => !selectedServices.value.find((ss) => ss.garageServiceId === service.garageServiceId))
		} catch (error) {
