			...
			<main class="main-content" style="margin-top: 70px; padding: 2rem">
				<div class="detail-header">
					...
					<div class="header-actions">
						<GmsButton v-if="canEdit" variant="primary" icon="fa-edit" @click="showEditDialog = true">Chỉnh sửa</GmsButton>
						<GmsButton v-if="canAssign" variant="success" icon="fa-user-plus" @click="showAssignDialog = true">Phân công thợ</GmsButton>
					</div>
				</div>
				...
				<div v-else-if="serviceTicket" class="detail-content">
					...
					<div class="info-card">
						<div class="card-header-with-action">
							...
							<GmsButton v-if="canEdit" variant="primary" size="small" icon="fa-plus" @click="showAddPartDialog = true">Thêm phụ tùng</GmsButton>
						</div>
						<GmsTable v-if="serviceTicket.parts && serviceTicket.parts.length > 0" :data="serviceTicket.parts" :columns="partsColumns" :pagination="false">
							<template #cell-actions="{ row }">
								<GmsButton v-if="canEdit" variant="danger" size="small" icon="fa-trash" @click="handleDeletePart(row.serviceTicketDetailId)">Xóa</GmsButton>
							</template>
						</GmsTable>
					...
					<div class="info-card">
						<div class="card-header-with-action">
							...
							<GmsButton v-if="canEdit" variant="primary" size="small" icon="fa-plus" @click="showAddServiceDialog = true">Thêm dịch vụ</GmsButton>
						</div>
						<GmsTable v-if="serviceTicket.garageServices && serviceTicket.garageServices.length > 0" :data="serviceTicket.garageServices" :columns="servicesColumns" :pagination="false">
							<template #cell-actions="{ row }">
								<GmsButton v-if="canEdit" variant="danger" size="small" icon="fa-trash" @click="handleDeleteService(row.serviceTicketDetailId)">Xóa</GmsButton>
							</template>
						</GmsTable>
						<div v-else class="empty-state">
							<i class="fas fa-inbox"></i>
			...
			<form @submit.prevent="handleAddPart">
				<div class="mb-3">
					...
					<select v-model="partForm.partId" class="form-select" required>
						...
						<option v-for="part in availableParts" :key="part.partId" :value="part.partId">{{ part.partName }} - Tồn kho: {{ part.partStock }} {{ part.partUnit }}</option>
					</select>
				</div>
				<GmsInput v-model.number="partForm.quantity" label="Số lượng" type="number" :min="1" :required="true" />
				<template #footer>
					<GmsButton variant="outline" @click="showAddPartDialog = false">Hủy</GmsButton>
					<GmsButton type="submit" variant="primary" :loading="addPartLoading">Thêm</GmsButton>
				</template>
			</form>
		</GmsDialog>
			...
			<form @submit.prevent="handleAddService">
				<div class="mb-3">
					...
					<select v-model="serviceForm.garageServiceId" class="form-select" required>
						...
						<option v-for="service in availableServices" :key="service.garageServiceId" :value="service.garageServiceId">
							{{ service.garageServiceName }} - {{ formatPrice(service.garageServicePrice) }}
						</option>
					</select>
				</div>
				<GmsInput v-model.number="serviceForm.quantity" label="Số lượng" type="number" :min="1" :required="true" />
				<template #footer>
					<GmsButton variant="outline" @click="showAddServiceDialog = false">Hủy</GmsButton>
					<GmsButton type="submit" variant="primary" :loading="addServiceLoading">Thêm</GmsButton>
				</template>
			</form>
		</GmsDialog>
...
<script setup>
	...
	import { SERVICE_TICKET_STATUS, SERVICE_TICKET_STATUS_COLORS, SERVICE_TICKET_STATUS_LABELS, TASK_STATUS_COLORS, TASK_STATUS_LABELS } from '@/constant/serviceTicketStatus'
	...
	onMounted(async () => {
		...
		await Promise.all([loadServiceTicket(), loadMechanics(), loadParts(), loadServices()])
	})
</script>
