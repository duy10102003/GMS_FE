<template>
	<div class="garage-services-view">
		...
		<div class="content-wrapper" :style="{ marginLeft: sidebarCollapsed ? '80px' : '260px' }">
			<TheHeader title="Quản lý dịch vụ garage" :show-search="false" :notifications="notifications" @logout="handleLogout" />

			<main class="main-content" style="margin-top: 70px; padding: 2rem">
				<!-- Toolbar -->
				<div class="toolbar">
					<div class="toolbar-left">
						<GmsInput v-model="searchQuery" placeholder="Tìm theo tên dịch vụ..." prefix-icon="fa-search" :clearable="true" class="search-input" @input="handleSearch" />
					</div>

					<div class="toolbar-right">
						<GmsButton variant="primary" icon="fa-plus" @click="openCreateDialog">Tạo dịch vụ mới</GmsButton>
					</div>
				</div>
				...
				<div class="filters">
					<div class="filter-group">
						<label>Giá từ:</label>
						<input v-model="filters.priceFrom" type="number" class="filter-input" placeholder="0" min="0" @change="applyFilters" />
					</div>

					<div class="filter-group">
						<label>Giá đến:</label>
						<input v-model="filters.priceTo" type="number" class="filter-input" placeholder="999999999" min="0" @change="applyFilters" />
					</div>
				...
				<div class="table-container">
					<GmsTable :data="services" :columns="tableColumns" title="Danh sách dịch vụ garage" :loading="loading" :pagination="false" :scrollable="true" @sort="handleSort">
				...
				<div v-if="totalItems > 0" class="pagination mt-4">
					<div class="pagination-left">
						<div class="pagination-info">Hiển thị {{ startIndex + 1 }}-{{ endIndex }} trong tổng {{ totalItems }} dịch vụ</div>
		...
		<!-- Create/Edit Dialog -->
		<GmsDialog v-model="showFormDialog" :title="isEditing ? 'Cập nhật dịch vụ' : 'Tạo dịch vụ mới'" size="medium">
			<form @submit.prevent="handleSubmit">
				<div class="mb-3">
					<label class="form-label">Tên dịch vụ *</label>
					<GmsInput v-model="formData.garageServiceName" placeholder="Nhập tên dịch vụ..." required :maxlength="255" />
				</div>

				<div class="mb-3">
					<label class="form-label">Giá dịch vụ (VNĐ)</label>
					<GmsInput v-model.number="formData.garageServicePrice" type="number" placeholder="Nhập giá dịch vụ..." :min="0" step="1000" />
		...
		<GmsDialog v-model="showDeleteDialog" title="Xác nhận xóa" size="small">
			<template v-if="selectedService">
				<p>
					...
					<strong>{{ selectedService.garageServiceName }}</strong>
					?
				</p>
