<template>
  <div class="manager-warranty-view">
      ...
      <main class="main-content">
        <div class="toolbar">
          <div class="toolbar-left">
            <GmsInput
              v-model="searchQuery"
              placeholder="Tìm theo mã phụ tùng, biển số, khách hàng..."
...
const getStatusLabel = (status) => {
  ...
  const handleRowClick = (row) => {
    const id = row.id || row.warrantyId || row.warrantyID
    if (!id) return
    console.log('Navigating to warranty detail with ID:', row.customerEmail)
    router.push({
...
const loadWarranties = async () => {
  try {
    ...
    const res = await warrantyService.getPaging(params)
