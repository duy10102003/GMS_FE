<template>
	<div class="customer-invoice-detail-view">
		...
		<div class="dashboard-customer-body">
			<TheHeader title="Chi tiết hóa đơn" :show-search="false" :notifications="notifications" @logout="handleLogout" />

			<main class="main-content">
				<section v-if="loading" class="card loading">Đang tải chi tiết hóa đơn...</section>
				...
				<section v-else-if="invoice" class="card">
					...
					<div class="card-body">
						<div class="info-grid">
							<div class="info-item">
								...
								<span class="value">{{ invoice.customer.customerName || 'N/A' }}</span>
							</div>
							<div class="info-item">
								<span class="label">Số điện thoại</span>
								<span class="value">{{ invoice.customer.customerPhone || 'N/A' }}</span>
							</div>
							<div class="info-item">
								<span class="label">Biển số xe</span>
								<span class="value">{{ invoice.vehicle.vehicleLicensePlate || 'N/A' }}</span>
							</div>
						</div>

						<h3 class="section-title">Phụ tùng</h3>
						<table class="detail-table">
							...
							<tbody>
								<tr v-for="(item, idx) in invoice.parts || []" :key="idx">
									<td>{{ item.part.partName }}</td>
									<td>{{ item.quantity || 1 }}</td>
									<td>{{ formatMoney(item.part.partPrice) }}</td>
									<!-- <td>{{ formatMoney(item.total || item.amount || 0) }}</td> -->
								</tr>
								<tr v-if="!(invoice.parts && invoice.parts.length)">
									<td colspan="4" class="text-center muted">Không có dữ liệu chi tiết hóa đơn</td>
								</tr>
							</tbody>
						</table>
						...
						<table class="detail-table">
							...
							<tbody>
								<tr v-for="(item, idx) in invoice.garageServices || []" :key="idx">
									<td>{{ item.garageService.garageServiceName || item.serviceName || item.partName }}</td>
									<!-- <td>{{ item.quantity || 1 }}</td> -->
									<td>{{ formatMoney(item.garageService.garageServicePrice || item.price || 0) }}</td>
									<!-- <td>{{ formatMoney(item.total || item.amount || 0) }}</td> -->
								</tr>
								<tr v-if="!(invoice.garageServices && invoice.garageServices.length)">
									<td colspan="4" class="text-center muted">Không có dữ liệu chi tiết hóa đơn</td>
								</tr>
							</tbody>
						</table>
					...
					<div class="card-amount">
						...
						<div class="info-item">
							<span class="label">Tổng thuế</span>
							<span class="value amount">
								{{ formatMoney(invoice.discountAmout || 0) }}
							</span>
						</div>
					...
					<footer class="card-footer">
						<GmsButton variant="outline" icon="fa-arrow-left" @click="goBack">Quay lại danh sách</GmsButton>

						<GmsButton v-if="(invoice.status ?? invoice.invoiceStatus) === 0" variant="primary" icon="fa-credit-card" :loading="paying" @click="handlePay">Thanh toán</GmsButton>
					</footer>
				</section>

				<section v-else class="card empty">Không tìm thấy hóa đơn.</section>
			</main>
		</div>
