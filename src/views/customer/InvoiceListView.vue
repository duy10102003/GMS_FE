					...
					>
						...
						<template #cell-actions="{ row }">
							<div class="action-buttons">
								<GmsButton variant="outline" size="small" icon="fa-eye" @click.stop="viewDetail(row)">
									Chi tiết
								</GmsButton>
								<GmsButton
									...
									size="small"
									icon="fa-credit-card"
									:loading="payingId === row.invoiceId"
									@click.stop="pay(row)"
								>
									Thanh toán
								</GmsButton>
...
const applyColumnFilter = () => {
	...
	columnFilters.value = columnFilters.value.filter(
		(f) => f.columnName !== columnKey && f.columnName !== currentFilterColumn.value.key
	)

	if (
		filterForm.value.operator !== 'empty' &&
		filterForm.value.operator !== 'not_empty' &&
		(filterForm.value.value || filterForm.value.value === 0)
	) {
...
const loadInvoices = async () => {
	try {
		...
		const res = await invoiceService.getPaging(params)
