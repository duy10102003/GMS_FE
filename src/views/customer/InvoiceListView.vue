<template>
	<div class="customer-invoices-view">
		<TheSideBar @logout="handleLogout" />
		<div class="dashboard-customer-body">
			<TheHeader title="Hóa đơn của tôi" :show-search="false" :notifications="notifications" @logout="handleLogout" />
			<main class="main-content">
				<!-- Toolbar -->
				<div class="toolbar">
					<div class="toolbar-left">
						<GmsInput v-model="searchQuery" placeholder="Tìm theo mã hóa đơn..." prefix-icon="fa-search" :clearable="true" class="search-input" @input="handleSearch" />
					</div>
				</div>
				<!-- Active Filters -->
					...
					<GmsTable :data="invoices" :columns="tableColumns" title="Danh sách hóa đơn" :loading="loading" :pagination="false" :scrollable="true" @sort="handleSort" @filter-click="openFilterModal">
						...
						<template #cell-actions="{ row }">
							<div class="action-buttons">
								...
								<GmsButton v-if="row.invoiceStatus === 0" variant="primary" size="small" icon="fa-credit-card" :loading="payingId === row.invoiceId" @click.stop="pay(row)">Thanh toán</GmsButton>
							</div>
						</template>
					</GmsTable>
				</div>
...
<script setup>
	...
	const tableColumns = ref([
		...
		{ key: 'status', label: 'Trạng thái', sortable: true, filterable: true, isNumeric: true },
	...
	const isNumericColumn = (column) => {
		return column?.isNumeric || ['status', 'totalAmount'].includes(column?.key)
	}
	...
	const viewDetail = (invoice) => {
		router.push(`/customer/invoices/${invoice.invoiceId}`)
	}

	const pay = async (invoice) => {
		try {
			payingId.value = invoice.invoiceId
	...
	const loadInvoices = async () => {
		try {
			...
			// Lấy userId từ auth store (Spring Boot login)
			...
			// Chỉ lấy hóa đơn của customer tương ứng với user đang đăng nhập
			const res = await invoiceService.getPagingByCustomer(userId, params)
			const items = res.data?.items || res.items || res.data || []

			invoices.value = items
			totalItems.value = res.data?.total || res.total || items.length
		} catch (error) {
...
<style scoped>
	...
	/* Tắt hiệu ứng hover phóng to / nhảy nút trong màn hóa đơn customer */
	.table-container :deep(.gms-button:hover) {
