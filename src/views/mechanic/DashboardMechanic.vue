			...
			<main class="main-content" style="padding: 2rem">
				...
				<div class="stats-grid">
					...
					<div class="stat-card">
						...
						<div class="change positive" v-if="stats.completedChange > 0">
							...
							+{{ stats.completedChange }} so tháng trước
						</div>
					</div>
				...
				<div class="content-card">
					<div class="content-card-header">
						...
						<GmsButton variant="primary" icon="fa-eye" @click="$router.push('/mechanic/tasks')">Xem tất cả</GmsButton>
					</div>
					...
					<div v-else-if="currentTasks.length > 0" class="tasks-list">
						<div v-for="task in currentTasks" :key="task.technicalTaskId" class="task-card" @click="$router.push(`/mechanic/tasks/${task.technicalTaskId}`)">
				...
				<div class="content-card">
					...
					<div v-if="recentCompleted.length > 0" class="tasks-list">
						<div v-for="task in recentCompleted" :key="task.technicalTaskId" class="task-card completed" @click="$router.push(`/mechanic/tasks/${task.technicalTaskId}`)">
							<div class="task-header">
								<div>
									<h6>Task #{{ task.technicalTaskId }}</h6>
									<span class="task-code">{{ task.serviceTicketCode || `#${task.serviceTicketId}` }}</span>
...
<script setup>
	...
	const loadDashboardData = async () => {
		try {
			...
			// Tính toán stats
			stats.value.inProgress = allTasks.filter((t) => t.serviceTicketStatus === SERVICE_TICKET_STATUS.IN_PROGRESS || t.serviceTicketStatus === 2).length

			stats.value.pendingConfirmation = allTasks.filter((t) => t.serviceTicketStatus === SERVICE_TICKET_STATUS.PENDING_TECHNICAL_CONFIRMATION || t.serviceTicketStatus === 0).length
