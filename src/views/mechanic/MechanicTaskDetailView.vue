...
<script setup>
	...
	const canStartInProgress = computed(() => {
		...
		return statusNum === SERVICE_TICKET_STATUS.ADJUSTED_BY_TECHNICAL || status === SERVICE_TICKET_STATUS.ADJUSTED_BY_TECHNICAL || status === 1 || status === '1'
	})
	...
	const handleAdjust = async () => {
		...
		try {
			...
			// Cập nhật service ticket với initialIssue và garageServiceIds
			const updatePayload = {
				modifiedBy,
				...
				garageServiceIds: selectedServices.value.map((s) => s.garageServiceId) // Gửi garageServiceIds
			}
