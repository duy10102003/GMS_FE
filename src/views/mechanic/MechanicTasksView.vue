<template>
	<div class="mechanic-tasks-view">
		...
		<div class="content-wrapper" :style="{ marginLeft: sidebarCollapsed ? '80px' : '260px' }">
			<TheHeader title="Danh sách công việc" :show-search="false" @logout="handleLogout" />
			...
			<main class="main-content" style="margin-top: 70px; padding: 2rem">
				...
				<div v-else class="tasks-grid">
					<div v-for="task in tasks" :key="task.technicalTaskId" class="task-card" @click="viewTaskDetail(task)">
						...
						<div class="task-footer">
							...
							<div class="task-actions">
								<GmsButton v-if="task.taskStatus === TASK_STATUS.PENDING" variant="primary" size="small" icon="fa-play" @click.stop="startTask(task)">Bắt đầu</GmsButton>
								<GmsButton v-else-if="task.taskStatus === TASK_STATUS.IN_PROGRESS" variant="success" size="small" icon="fa-check" @click.stop="confirmTask(task)">Hoàn thành</GmsButton>
								<GmsButton variant="outline" size="small" icon="fa-eye" @click.stop="viewTaskDetail(task)">Chi tiết</GmsButton>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	import { SERVICE_TICKET_STATUS, TASK_STATUS, TASK_STATUS_COLORS, TASK_STATUS_LABELS } from '@/constant/serviceTicketStatus'
